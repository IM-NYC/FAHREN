...
<#
.SYNOPSIS
    Novaflow CLI for training and evaluating MNIST models on Windows.
#>
...
$ConfigDir  = Join-Path $env:USERPROFILE ".nova"
$ConfigFile = Join-Path $ConfigDir "config.ini"
$CliExe     = Join-Path $RepoRoot "build\tools\Release\nova_cli.exe"
if (-not (Test-Path $CliExe)) {
    $CliExe = Join-Path $RepoRoot "build\tools\nova_cli.exe"
}
...
function Show-Header {
    $cfg = Read-Config
    Write-Host ""
    Write-Host "  Novaflow" -ForegroundColor Cyan
...
function Invoke-Cli($Command) {
    if (-not (Test-Path $CliExe)) {
        Write-Host "  Building nova_cli..." -ForegroundColor Yellow
        $cmake = Get-Command cmake -ErrorAction SilentlyContinue
        if (-not $cmake) { throw "cmake not found" }
        & cmake --build (Join-Path $RepoRoot "build") --config Release --target nova_cli
    }
